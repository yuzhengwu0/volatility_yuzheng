...
% 5) 在每个时间 bin 上，用 Noise ~ |CumEv| 回归，取 residual cumulative noise
% 6) 把时间分成 Early / Mid / Late 三段，用
%    CumEv(E/M/L) + ResidualCumNoise(E/M/L) 做 logistic 回归预测 correct
...
% 根据你之前的设定：<=100 低，一般是 50/100；>=101 高，一般是 200/400
coh = allStruct.rdm1_coh(:);             
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 局部函数：对一组 trial index 做 cumulative noise + residual 分析
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if isempty(y_correct)
    warning('No valid trials in block "%s".', label_str);
    return;
end

%% 2. 在每个时间 bin 上回归掉 cumulative evidence 的影响，得到 residual cumulative noise

resid_CumNoise = nan(size(CumNoise_norm));   % trial x time

for b = 1:nBins
    y = CumNoise_norm(:, b);         % 当前时间点的 cumulative noise
    x = abs(CumEv_norm(:, b));       % 当前时间点的 |cumulative evidence|
    
    mask = ~isnan(y) & ~isnan(x);
    if sum(mask) < 10
        continue;                    % 有效 trial 太少就算了
    end
    
    X     = [ones(sum(mask),1), x(mask)];  % [常数项, |cumEv|]
    beta  = X \ y(mask);                  % 最小二乘: y = b0 + b1 * |x|
    y_hat = X * beta;                     % “正常”应该有的 noise
    
    tmp        = nan(size(y));
    tmp(mask)  = y(mask) - y_hat;         % residual = 实际 noise - 正常 noise
    resid_CumNoise(:, b) = tmp;          % 额外 cumulative noise
end

%% 3. 把 0–1 时间轴上数据分成 Early / Mid / Late，取平均做 predictor
...
ResCumNoise_Early = mean(resid_CumNoise(:, early_idx), 2, 'omitnan');
ResCumNoise_Mid   = mean(resid_CumNoise(:, mid_idx),   2, 'omitnan');
ResCumNoise_Late  = mean(resid_CumNoise(:, late_idx),  2, 'omitnan');

% 设计矩阵：Early / Mid / Late 的 cumulative evidence + residual cumulative noise
X = [CumEv_Early, CumEv_Mid, CumEv_Late, ...
     ResCumNoise_Early, ResCumNoise_Mid, ResCumNoise_Late];
...
% logistic 回归：correct ~ cumEv(E/M/L) + residualCumNoise(E/M/L)
...
for i = 1:numel(pred_names)
    fprintf('%20s: beta = %+ .4e,  SE = %.4e,  p = %.4g\n', ...
        pred_names{i}, b(i), stats.se(i), stats.p(i));
end

%% 4. 画直方图：看 residual cumulative noise 在 correct / incorrect 里的分布（E/M/L）

idxC = (y_correct == 1) & valid_mask;
idxI = (y_correct == 0) & valid_mask;

figure('Name', ['Residual cumulative noise vs correctness - ' label_str]);

subplot(3,1,1);
histogram(ResCumNoise_Early(idxC), 'Normalization','pdf'); hold on;
histogram(ResCumNoise_Early(idxI), 'Normalization','pdf');
xlabel('Residual cumulative noise (Early)');
ylabel('Density');
legend({'Correct','Incorrect'});
title(['Early residual cumulative noise vs correctness (' label_str ')']);
grid on;

subplot(3,1,2);
histogram(ResCumNoise_Mid(idxC), 'Normalization','pdf'); hold on;
histogram(ResCumNoise_Mid(idxI), 'Normalization','pdf');
xlabel('Residual cumulative noise (Mid)');
ylabel('Density');
legend({'Correct','Incorrect'});
title(['Mid residual cumulative noise vs correctness (' label_str ')']);
grid on;

subplot(3,1,3);
histogram(ResCumNoise_Late(idxC), 'Normalization','pdf'); hold on;
histogram(ResCumNoise_Late(idxI), 'Normalization','pdf');
xlabel('Residual cumulative noise (Late)');
ylabel('Density');
legend({'Correct','Incorrect'});
title(['Late residual cumulative noise vs correctness (' label_str ')']);
grid on;
